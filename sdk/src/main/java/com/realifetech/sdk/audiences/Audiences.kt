...
import com.realifetech.BelongsToAudienceQuery
...
class Audiences private constructor() {
    ...
    fun deviceIsMemberOfAudience(audienceId: String, callback: (error: Error?, result: Boolean) -> Unit) {
        GlobalScope.launch(Dispatchers.IO) {
            ...
            val responseServer = apolloClient.query(BelongsToAudienceQuery(audienceId)).await()
            val belongsToAudienceResponse = responseServer.data?.me?.device?.belongsToAudience ?: false
